  ...
  return (
    <div className='space-y-4'>
      ...
      <Card title='Results History'>
        <Table
          columns={[
            { key: 'exam', label: 'Exam', render: (row) => (row as { exam: { name: string } }).exam.name },
            { key: 'status', label: 'Status', render: (row) => <Badge status={(row as { status: string }).status} /> },
            { key: 'remarks', label: 'Remarks', render: (row) => (row as { remarks?: string }).remarks || '-' },
          ]}
      ...
      <Card title='Transfer Class'>
        <div className='flex gap-3'>
          <select className='rounded border px-3 py-2' value={nextClassId} onChange={(event) => setNextClassId(event.target.value)}>
            ...
            {(classes.data?.data as Array<{ id: string; name: string; section: string }> | undefined)?.map((classItem) => (
      ...
      <Modal open={openConfirm} onClose={() => setOpenConfirm(false)} title='Confirm Transfer' footer={<><Button variant='secondary' onClick={() => setOpenConfirm(false)}>Cancel</Button><Button onClick={handleTransfer} loading={transfer.isPending}>Confirm</Button></>}>
