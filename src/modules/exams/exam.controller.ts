...
const getIp = (req: Request): string | undefined => req.ip || undefined;
...
const parseStatus = (value: unknown): ExamWorkflowStatus | undefined => {
  ...
  return ['DRAFT', 'REVIEW', 'APPROVED', 'PUBLISHED'].includes(normalized)
    ? (normalized as ExamWorkflowStatus)
    : undefined;
};
...
export const createExam = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await examService.createExam(payload, getUserId(req), getIp(req));
...
export const updateExam = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await examService.updateExam(String(req.params.id), payload, getUserId(req), getIp(req));
...
export const deleteExam = async (req: Request, res: Response, next: NextFunction) => {
  try {
    await examService.deleteExam(String(req.params.id), getUserId(req), getIp(req));
...
export const submitReview = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const data = await examService.submitReview(String(req.params.id), getUserId(req), getIp(req));
...
export const approveExam = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const data = await examService.approveExam(String(req.params.id), getUserId(req), getIp(req));
    return res.json(success(data, 'Exam approved'));
  } catch (error) {
    return next(error);
  }
};
...
export const rejectExam = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await examService.rejectExam(String(req.params.id), payload, getUserId(req), getIp(req));
    return res.json(success(data, 'Exam rejected'));
  } catch (error) {
    return next(error);
  }
};
...
export const publishExam = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const data = await examService.publishExam(String(req.params.id), getUserId(req), getIp(req));
    return res.json(success(data, 'Exam published'));
  } catch (error) {
    return next(error);
  }
};
